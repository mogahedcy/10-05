...
async function getProject(id: string) {
  try {
    ...
    // التحقق مما إذا كان المعرف بتنسيق صحيح (CUID أو UUID) لتجنب أخطاء Prisma
    const isPossibleId = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(decodedId) || /^[a-z0-9]{20,30}$/i.test(decodedId);

    const whereClause = isPossibleId 
...
export async function generateMetadata({ params }: { params: Promise<{ id: string; locale: string }> }): Promise<Metadata> {
  ...
  if (!project) {
    return {
      title: isEn ? 'Project Not Found | Deyar Jeddah Global' : 'المشروع غير موجود | ديار جدة العالمية',
  ...
  const dynamicKeywords = [
    ...
    project.location,
    ...projectKeywords,
    isEn ? 'Deyar Jeddah Global' : 'ديار جدة العالمية'
  ].filter(Boolean);

  const seoTitle = project.metaTitle || (isEn ? `${project.title} | Deyar Jeddah Global` : `${project.title} | ديار جدة العالمية`);
  ...
  return {
    ...
    openGraph: {
      ...
      type: 'article',
      publishedTime: (project.createdAt || new Date()).toISOString(),
      modifiedTime: (project.updatedAt || project.createdAt || new Date()).toISOString(),
      authors: [isEn ? 'Deyar Jeddah Global' : 'ديار جدة العالمية'],
