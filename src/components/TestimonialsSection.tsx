...
export default function TestimonialsSection({ initialReviews = [] }: { initialReviews?: Review[] }) {
  const locale = useLocale();
  const t = useTranslations('testimonials');
  const isRTL = locale === 'ar';
  const localePath = locale === 'ar' ? '' : '/en';

  const defaultTestimonials = [0, 1, 2, 3, 4, 5].map((index) => ({
    id: `fallback-${index + 1}`,
  ...
  const testimonials = initialReviews && initialReviews.length > 0 ? initialReviews : defaultTestimonials;
